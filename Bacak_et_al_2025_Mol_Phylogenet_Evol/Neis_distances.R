...
vcf <- read.vcfR("Erysimum.odoratum_karpatske.filtered.DP8.passed.m02.inRegs.bezOutgrps.vcf.gz")

genlight <- vcfR2genlight.hexa.MK(vcf) # run function at the end of the script

pop(genlight)<-c("IZVR", "LPSH", "Jasik", "Jasik", ..............,"ZARN","ZEIH")
...
stamppPhylip(D.ind, file="Erysimum.odoratum_karpatske.filtered.DP8.passed.m02.inRegs.bezOutgrps.indiv_Neis_distance.dst") # export matrix - for SplitsTree

aa.D.pop <- stamppNeisD(aa.genlight, pop = TRUE)   # Nei's 1972 distance between pops
stamppPhylip(aa.D.pop, file="Erysimum.odoratum_karpatske.filtered.DP8.passed.m02.inRegs.bezOutgrps.pops_Neis_distance.dst") # export matrix - for SplitsTree
