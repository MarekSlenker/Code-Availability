...
# MK - upraveny skript, aby vzdy vratil 2 alely
# TODO: ak je len 1 blok, tak Automatick bere celu sekvenciu. PROBLEM: sekvencia moze mat aj nesfazovane snipy mimo jedneho sfazovaneho bloku.
...
def getLargestPhaseBlock(ranges,seqLength):
    ...
    longestblock = -1  # MK, inac bolo 1; 12.10.2020
    for r in range(len(ranges)):
                                    # MK 14.10.2020 - zjednoduchsujem cast hladania najdlhsieho bloku.
        start = ranges[r][0]
        end = ranges[r][1]
        
        # MK 14:10:2020  -  MOSMATTERS + MK, prepisane a nakoniec zmenene, tento if je len ako {} :)
        if (False):
            if r == 0:
                # start = 1   
                start = ranges[r][0]  # MK
...
# def replace_with_ref(seq1,ref,phaseBlock,allele): # BEZ _h1..
def replace_with_ref(seq1,ref,phaseBlock):
    # MK: V tomto mieste mi to vracalo iba jednu, ked boli rovnake. ale ja chcem vzdy 2, oznacene ako H1 a H2. tie budem dalej rozoberat, kazdy haplotyp ako samostatna alela vzorky
    # if seq1.seq == seq2.seq:
    #     return [SeqRecord(ref.seq,id=ref.id,description='')]
    # else:
    # haplo1 = insertPhase(ref,seq1,phaseBlock,"{}_h".format(seq1.id)+allele) # BEZ _h1..
    haplo1 = insertPhase(ref,seq1,phaseBlock,"{}".format(seq1.id))
    return [haplo1]
...
def main():
    ...
    parser.add_argument("haplotype_files",help="FASTA file containing sequences for one genes",nargs="+")
    ...
    parser.add_argument("--allele",help="numb of allele to process", type=str, required=True) 
    args = parser.parse_args()
    
    if len(args.haplotype_files) != 1:
        print("Please supply exactly ONE haplotype FASTA file!\n")
        sys.exit(1)
    ...
    # DEBUG INFI:
    # geneList = haplotyp, fazovany subor
    # 
    ...
    for gene in geneList:
        if gene in gtf_dict:  # ak je gen v GTF subore - ma aspon 1 blok. ak ma len 1 snip, tak ten sa nefazuje
            if gene in ref_dict:
                ...
                if args.edit == 'ref':
                    #seqs_to_write += replace_with_ref(haplotype1_dict[gene],ref_dict[gene],phaseBlock,args.allele) # BEZ _h1..
                    seqs_to_write += replace_with_ref(haplotype1_dict[gene],ref_dict[gene],phaseBlock)
                # elif args.edit == "delete":
                #     seqs_to_write += delete_extra(haplotype1_dict[gene],haplotype2_dict[gene],ref_dict[gene],phaseBlock)
        else:
            # MK: Ma len 1 snip. ked ma 1 snip, tak nie je v GTF suvore, no je v v1 a v2. phased.fasta. Cize chem tieto sfazovane, a nie IUPAC.
            # predtym som bral IUPAC, ale te nechcem, te su dobre len na maskovanie
            # nie je pravda, ak nie je v GTF, moze mat aj viac nesfazovanych snipov !!
            #seqs_to_write += [SeqRecord(haplotype1_dict[gene].seq, id=(gene + "_h" + args.allele), description='')] # BEZ _h1..
            seqs_to_write += [SeqRecord(haplotype1_dict[gene].seq, id=gene, description='')]
            # ked chces IUPAC? pouzi TOTO:
            # if gene in ref_dict:
            #   seqs_to_write += [SeqRecord(ref_dict[gene].seq, id=(gene + "_h1"), description='')]
            #   seqs_to_write += [SeqRecord(ref_dict[gene].seq, id=(gene + "_h2"), description='')]
